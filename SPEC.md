...
|--------|-----------------|---------------|
| **Optolith Database Schema** (`elyukai/optolith-database-schema`) | Complete JSON Schema + TypeScript type definitions for all DSA5 entities (characters, spells, liturgies, talents, SFs, equipment). Database itself is closed-source but schema is public. | Primary reference for character import format. Use TypeScript definitions to validate our data models match Optolith's structure exactly. |
